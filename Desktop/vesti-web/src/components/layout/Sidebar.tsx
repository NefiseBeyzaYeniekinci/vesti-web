...
import { LogOut, Settings, Shirt, Sparkles, Store, MessageCircle, Heart } from "lucide-react";

export function Sidebar() {
    const navItems = [
        { name: "Gardırop", href: "/wardrobe", icon: <Shirt className="w-5 h-5" /> },
        { name: "Kombin Önerileri", href: "/suggestions", icon: <Sparkles className="w-5 h-5" /> },
    ...
    return (
        <aside className="w-64 h-[calc(100vh-4rem)] border-r bg-vesti-background flex flex-col justify-between hidden md:flex sticky top-16">
            ...
            <div className="py-6 flex flex-col gap-2 px-4 border-t">
                <Link href="/style-profile" className="flex items-center gap-3 px-4 py-2 rounded-lg text-vesti-text hover:bg-vesti-text/10 transition-colors">
                    <Heart className="w-5 h-5" />
                    <span className="font-medium">Tarz Profili</span>
                </Link>
