...
import { SwapOfferModal } from '@/components/marketplace/SwapOfferModal';
                        ...
                        <div className="border-t border-gray-100 pt-8 mt-auto">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 w-full">
                                {isLoggedIn && session?.user?.id === item.seller.id ? (
                                    ...
                                    <>
                                        <Link href={`/login?callbackUrl=/marketplace/${item.id}`} className="flex-1">
                                            <button className="w-full bg-indigo-600 text-white font-medium py-3.5 px-6 rounded-xl hover:bg-indigo-700 transition-colors shadow-sm active:scale-[0.98]">
                                                Sipariş Ver (Giriş Yap)
                                            </button>
                                        </Link>
                                        <Link href={`/login?callbackUrl=/marketplace/${item.id}`} className="flex-1">
                                            <button className="w-full border border-gray-300 text-gray-700 font-medium py-3.5 px-6 rounded-xl hover:bg-gray-50 transition-colors shadow-sm active:scale-[0.98] text-center">
                                                Mesaj Gönder (Giriş Yap)
                                            </button>
                                        </Link>
                                        {item.isSwapOpen && (
                                            <Link href={`/login?callbackUrl=/marketplace/${item.id}`} className="flex-1">
                                                <button className="w-full bg-emerald-600 text-white font-medium py-3.5 px-6 rounded-xl hover:bg-emerald-700 transition-colors shadow-sm active:scale-[0.98] text-center">
                                                    Takas Teklif Et (Giriş Yap)
                                                </button>
                                            </Link>
                                        )}
                                    </>
