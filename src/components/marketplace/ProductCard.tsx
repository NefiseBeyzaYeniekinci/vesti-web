...
export function ProductCard({ item, initialIsFavorited = false }: { item: MarketplaceItem; initialIsFavorited?: boolean }) {
    ...
    return (
        <Link href={`/marketplace/${item.id}`} className="group block bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden relative">
            {/* Görsel ve Etiketler */}
            <div className="relative aspect-[4/5] bg-gray-50 overflow-hidden">
                ...
                {/* Favori Butonu */}
                <FavoriteButton listingId={item.id} initialIsFavorited={initialIsFavorited} />
